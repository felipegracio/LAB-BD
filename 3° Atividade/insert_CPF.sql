...
BEGIN
    ...
    WHILE(@aux <= LEN(@cpf))
    BEGIN
        IF(@aux = 1) SET @digito_anterior = SUBSTRING(@cpf, @aux, 1)
		ELSE SET @digito_anterior = SUBSTRING(@cpf, @aux-1, 1)

		SET @digito_atual = SUBSTRING(@cpf, @aux, 1)
        ...
        IF(@aux = 11 AND @digitos_identicos = 1) SET @cpf_valido = 0
...
BEGIN
  DECLARE @aux INT,
	...
	BEGIN
		...
		WHILE (@aux <= 9)
		BEGIN
		  SET @soma = @soma + CONVERT(INT,SUBSTRING(@CPF, @aux, 1)) * (11 - @aux);
		  SET @aux = @aux + 1
		END
		...
		WHILE (@aux <= 10)
		BEGIN
			SET @soma = @soma + CONVERT(INT,SUBSTRING(@CPF, @aux, 1)) * (12 - @aux);
			SET @aux = @aux + 1
		END
...
DECLARE @output BIT
EXEC sp_insert_cadastro '61638954022', 'Paulo', 'Rua America', 300, @output OUTPUT
PRINT @output
